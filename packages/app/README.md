...
Draw something, then press `Enter` or `Ctrl+S` to write the result to stdout. In Select mode, drag a marquee around objects and press `C`, `X`, or `V` to copy, cut, or paste them. `Ctrl+Shift+C/X/V` are aliases when the terminal forwards them; `Ctrl+C` remains the terminal cancel shortcut.
