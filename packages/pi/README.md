...
Use `Enter` or `Ctrl+S` to insert the drawing into Pi. In Select mode, press `C`, `X`, or `V` to copy, cut, or paste selected objects; `Ctrl+Shift+C/X/V` are optional aliases when forwarded by the terminal. Use `Ctrl+C` or `Ctrl+Q` to close without inserting.
